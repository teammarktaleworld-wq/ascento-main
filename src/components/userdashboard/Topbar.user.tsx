// // src\components\userdashboard\Topbar.user.tsx
...
export default function TopBar({
  ...
  const markRead = async (id: string) => {
    ...
    await p;
    setNotifications(ns => ns.map(n => n.id === id ? { ...n, isRead: true } : n));
    setUnreadCount(c => { const next = Math.max(0, c - 1); prevCountRef.current = next; return next; });
  };
  ...
  return (
    <>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-6 py-3.5 bg-white border-b border-gray-100 shadow-[0_1px_0_rgba(0,0,0,0.04)]">
        <div className="flex items-center gap-3">
          {/* Hamburger — visible on mobile */}
          <button
            onClick={onMenuClick}
            className="lg:hidden w-9 h-9 rounded-xl bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 transition-colors"
          >
            <Menu size={18} />
          </button>
          <h2 className="text-lg font-black text-[#1A1A2E] tracking-tight">{label}</h2>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <Bell size={18} className={ringing ? "bell-ringing" : ""} />
              {unreadCount > 0 && (
                <span key={unreadCount} className="badge-pop absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1
                  bg-gradient-to-br from-[#FF6B6B] to-[#FF4444] text-white text-[10px]
                  font-black rounded-full flex items-center justify-center leading-none shadow-sm">
                  {unreadCount > 99 ? "99+" : unreadCount}
                </span>
                    ...
                    notifications.map(n => {
                      ...
                      return (
                        <div key={n.id} onClick={() => !n.isRead && markRead(n.id)}
                          className={`flex items-start gap-3 px-4 py-3 cursor-pointer transition-colors
                            ${n.isRead ? "hover:bg-gray-50/60" : "bg-[#FFFDF7] hover:bg-orange-50/60 border-l-2"}`}
                          style={!n.isRead ? { borderLeftColor: meta.color } : {}}
                        >
                          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-base"
                            style={{ background: meta.bg }}>
