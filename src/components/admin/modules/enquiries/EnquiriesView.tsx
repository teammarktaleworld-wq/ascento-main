...
const ENQUIRY_STATUSES = ["New", "Follow-up", "Enrolled", "Closed"];
const CAMP_STATUSES    = ["New", "Contacted", "Confirmed", "Cancelled"];
...
type TabType = "contact" | "crm" | "summercamp";
...
export default function EnquiriesView() {
  ...
  const [enquiriesData,       setEnquiriesData]       = useState<any[]>([]);
  const [contactEnquiriesData,setContactEnquiriesData] = useState<any[]>([]);
  const [campEnrollmentsData, setCampEnrollmentsData]  = useState<any[]>([]);
  const [loading,        setLoading]        = useState(true);
  const [updatingStatus, setUpdatingStatus] = useState(false);

  // UI
  const [enquiryTab,     setEnquiryTab]     = useState<TabType>("contact");
  const [enquiryFilter,  setEnquiryFilter]  = useState("All");
  const [searchQuery,    setSearchQuery]    = useState("");
  const [selectedEnquiry,setSelectedEnquiry]= useState<any>(null);
  const [toast,          setToast]          = useState<string | null>(null);
  ...
  const fetchEnquiries = useCallback(async () => {
    ...
    try {
      ...
      setEnquiriesData(Array.isArray(crm)    ? crm    : []);
      setContactEnquiriesData(Array.isArray(contact) ? contact : []);
      setCampEnrollmentsData(Array.isArray(camp)    ? camp    : []);
    } catch {
      showToast("Failed to load enquiries");
    }
    setLoading(false);
  ...
  // ── Status update ────────────────────────────────────────────────
  const handleUpdateStatus = async (id: string, status: string, type: TabType) => {
    setUpdatingStatus(true);
    try {
      if (type === "summercamp") {
        await apiFetch(`/api/admin/summer-camp-enrollments?id=${id}`, {
          method: "PATCH",
          body: JSON.stringify({ status }),
        });
      } else {
  ...
  const handleDelete = async (id: string, type: TabType) => {
    ...
    try {
      if (type === "crm") {
        ...
        await apiFetch(`/api/admin/summer-camp-enrollments?id=${id}`, { method: "DELETE" });
      }
  ...
  const rawList = useMemo(() => {
    if (enquiryTab === "contact")    return contactEnquiriesData;
    if (enquiryTab === "crm")        return enquiriesData;
    return campEnrollmentsData;
  }, [enquiryTab, contactEnquiriesData, enquiriesData, campEnrollmentsData]);

  const filteredEnquiries = useMemo(() => {
    return rawList
      .filter(e => {
        const searchTarget = `${e.name||''} ${e.studentName||''} ${e.childName||''} ${e.parentName||''} ${e.phone||''} ${e.email||''}`.toLowerCase();
        const matchesSearch = !searchQuery || searchTarget.includes(searchQuery.toLowerCase());
        const matchesFilter =
          enquiryTab === "contact" ||
          enquiryFilter === "All"  ||
  ...
  }, [rawList, searchQuery, enquiryFilter, enquiryTab]);
  ...
  const campStats = useMemo(() => ({
    total:     campEnrollmentsData.length,
    new:       campEnrollmentsData.filter(e => e.status === "New").length,
    confirmed: campEnrollmentsData.filter(e => e.status === "Confirmed").length,
    programs:  [...new Set(campEnrollmentsData.map(e => e.program))].length,
  }), [campEnrollmentsData]);

  // ── Active status set ────────────────────────────────────────────
  const activeStatuses = enquiryTab === "summercamp" ? CAMP_STATUSES : ENQUIRY_STATUSES;

  // ── Helper: display name ─────────────────────────────────────────
  const displayName = (e: any) =>
    e.childName || e.name || e.studentName || "—";

  // ── Tabs config ──────────────────────────────────────────────────
  const tabs: { id: TabType; label: string }[] = [
    { id: "contact",    label: "📬 Website Forms" },
    ...
    <div className="space-y-6 animate-in fade-in duration-500 relative pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-medium">
            Manage website forms, CRM leads, and summer camp enrolments.
          </p>
        </div>
        <button
          onClick={fetchEnquiries}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {enquiryTab === "crm" && <>
          <Card className="p-6 group hover:-translate-y-1 transition-transform duration-300">
            ...
            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Total Leads</p>
            <h3 className="text-3xl font-black text-[#1A1A2E] my-1">{crmStats.total}</h3>
            <p className="text-xs text-gray-500 font-medium">All recorded leads</p>
          </Card>
          <Card className="p-6 group hover:-translate-y-1 transition-transform duration-300">
        ...
        {enquiryTab === "summercamp" && <>
          <Card className="p-6 group hover:-translate-y-1 transition-transform duration-300">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#FF6B6B] to-[#FFB347] flex items-center justify-center text-white mb-4 shadow-[0_4px_15px_rgba(255,107,107,0.3)]"><Sun size={20} /></div>
            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Total Enrolments</p>
            <h3 className="text-3xl font-black text-[#1A1A2E] my-1">{campStats.total}</h3>
            <p className="text-xs text-gray-500 font-medium">All camp registrations</p>
          </Card>
          <Card className="p-6 group hover:-translate-y-1 transition-transform duration-300">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#4ECDC4] to-[#45B7AA] flex items-center justify-center text-white mb-4 shadow-[0_4px_15px_rgba(78,205,196,0.3)]"><AlertCircle size={20} /></div>
            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">New</p>
            <h3 className="text-3xl font-black text-[#1A1A2E] my-1">{campStats.new}</h3>
            <p className="text-xs text-gray-500 font-medium">Awaiting contact</p>
          </Card>
          <Card className="p-6 group hover:-translate-y-1 transition-transform duration-300">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#A78BFA] to-[#7C3AED] flex items-center justify-center text-white mb-4 shadow-[0_4px_15px_rgba(167,139,250,0.3)]"><CheckCircle2 size={20} /></div>
            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Confirmed</p>
            <h3 className="text-3xl font-black text-[#1A1A2E] my-1">{campStats.confirmed}</h3>
            <p className="text-xs text-gray-500 font-medium">Seats locked in</p>
          </Card>
          <Card className="p-6 group hover:-translate-y-1 transition-transform duration-300">
            <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#FFB347] to-[#FFD700] flex items-center justify-center text-white mb-4 shadow-[0_4px_15px_rgba(255,179,71,0.3)]"><BookOpen size={20} /></div>
            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Programs</p>
            <h3 className="text-3xl font-black text-[#1A1A2E] my-1">{campStats.programs}</h3>
            <p className="text-xs text-gray-500 font-medium">Unique programs chosen</p>
          </Card>
        </>}
      </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex p-1.5 bg-white rounded-2xl border border-[#F0EEF8] shadow-sm w-full sm:w-auto">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => {
                setEnquiryTab(tab.id);
                setSelectedEnquiry(null);
                setEnquiryFilter("All");
                setSearchQuery("");
              }}
              className={`flex-1 sm:flex-none px-5 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 ${
                enquiryTab === tab.id
                  ? "bg-gradient-to-r from-[#FF6B6B] to-[#FFB347] text-white shadow-md"
                  : "text-gray-500 hover:bg-gray-50 hover:text-[#1A1A2E]"
              }`}
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          {/* Status filter — show for crm and summercamp */}
          {enquiryTab !== "contact" && (
            <div className="flex gap-2 overflow-x-auto pb-1 sm:pb-0">
      ...
      {/* MAIN SPLIT LAYOUT */}
      <div className={`grid gap-6 items-start transition-all duration-500 ${selectedEnquiry ? "lg:grid-cols-[1fr_420px]" : "grid-cols-1"}`}>
        ...
        <Card className="overflow-hidden min-h-[400px]">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center p-8">
              ...
              <p className="text-lg font-bold text-[#1A1A2E] mb-1">No records found</p>
              <p className="text-sm text-gray-500">No entries match your current filters.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#FFFDF7] border-b border-[#F0EEF8]">
                  <tr>
                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">
                      {enquiryTab === "summercamp" ? "Child / Parent" : "Name"}
                    </th>
                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">
                      {enquiryTab === "contact"    ? "Reason"   :
                       enquiryTab === "crm"        ? "Course"   : "Program"}
                    </th>
                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">Phone</th>
                    <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">
                      {enquiryTab === "contact" ? "Submitted" : "Status"}
                    </th>
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${gradients[i % 3]} text-white flex items-center justify-center text-sm font-bold flex-shrink-0 shadow-sm`}>
                              {name[0]?.toUpperCase()}
                            </div>
                            <div>
                              <p className={`text-sm font-bold transition-colors ${
                              ...
                              <p className="text-xs text-gray-400 font-medium">
                                {enquiryTab === "summercamp"
                                  ? (e.parentName ? `Parent: ${e.parentName}` : "No parent name")
                                  : (e.email || "No email")}
                              </p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 text-sm font-bold text-gray-600 max-w-[160px] truncate">
                          {e.reason || e.course || e.program || "—"}
                        </td>
                        <td className="px-6 py-4 text-sm font-bold text-[#1A1A2E]">
                          {e.phone || "—"}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            onClick={ev => { ev.stopPropagation(); handleDelete(e.id, enquiryTab); }}
                            className="p-2 text-gray-400 hover:text-[#FF6B6B] bg-white border border-[#F0EEF8] rounded-xl hover:border-[#FF6B6B]/30 hover:bg-[#FF6B6B]/10 transition-all shadow-sm opacity-0 group-hover:opacity-100"
                            title="Delete"
                          >
                            <Trash2 size={16} />
          ...
          <Card className="sticky top-24 animate-in slide-in-from-right-8 duration-300">
            ...
            <div className="p-6 border-b border-[#F0EEF8] bg-gradient-to-r from-[#FF6B6B]/5 to-[#FFB347]/5 flex justify-between items-center">
              <h3 className="text-lg font-black text-[#1A1A2E]">
                {selectedEnquiry._type === "summercamp" ? "Camp Enrolment" : "Enquiry Details"}
              </h3>
              <button
                onClick={() => setSelectedEnquiry(null)}
                className="p-1.5 text-gray-400 hover:text-[#FF6B6B] hover:bg-white rounded-lg transition-colors"
            ...
            <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#FF6B6B] to-[#FFB347] flex items-center justify-center text-white font-black text-xl shadow-md">
                  {displayName(selectedEnquiry)[0]?.toUpperCase()}
                </div>
                <div>
                  <h4 className="text-lg font-black text-[#1A1A2E] leading-tight">
                    {displayName(selectedEnquiry)}
                  </h4>
                  <p className="text-sm text-gray-500 font-medium mt-0.5">
                    {selectedEnquiry._type === "summercamp"
                      ? (selectedEnquiry.parentName ? `Parent: ${selectedEnquiry.parentName}` : "No parent name")
                      : (selectedEnquiry.email || "No email provided")}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                {[
                  { icon: Phone,   label: "Phone",   value: selectedEnquiry.phone || "—" },
                  ...(selectedEnquiry.email ? [{ icon: Mail, label: "Email", value: selectedEnquiry.email }] : []),

                  // Summer camp specific
                  ...(selectedEnquiry._type === "summercamp" ? [
                    { icon: Baby,     label: "Age Group", value: selectedEnquiry.ageGroup || "—" },
                    { icon: BookOpen, label: "Program",   value: selectedEnquiry.program  || "—" },
                  ] : []),

                  // Contact / CRM specific
                  ...(selectedEnquiry.reason   ? [{ icon: MessageCircle, label: "Reason",    value: selectedEnquiry.reason }]    : []),
                  ...(selectedEnquiry.course   ? [{ icon: BookOpen,      label: "Course",    value: selectedEnquiry.course }]    : []),
                  ...(selectedEnquiry.childAge ? [{ icon: Baby,          label: "Child Age", value: selectedEnquiry.childAge }]  : []),
                  ...(selectedEnquiry.source   ? [{ icon: Search,        label: "Source",    value: selectedEnquiry.source }]    : []),
              ...
              {/* Status Updater — crm + summercamp */}
              {(selectedEnquiry._type === "crm" || selectedEnquiry._type === "summercamp") && (
                <div className="pt-4 border-t border-[#F0EEF8]">
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-3">
                    Update Status
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    {(selectedEnquiry._type === "summercamp" ? CAMP_STATUSES : ENQUIRY_STATUSES).map(s => {
                      const isActive = selectedEnquiry.status === s;
                      const color = statusColor[s];
                      return (
                        <button
                          key={s}
                          disabled={updatingStatus || isActive}
                          onClick={() => handleUpdateStatus(selectedEnquiry.id, s, selectedEnquiry._type)}
                          style={{
                            backgroundColor: isActive ? color : "white",
                            borderColor:     isActive ? color : "#F0EEF8",
                            color:           isActive ? "white" : color,
              ...
              <div className="pt-4 border-t border-[#F0EEF8] space-y-2">
                ...
                <button
                  onClick={() => handleDelete(selectedEnquiry.id, selectedEnquiry._type)}
                  className="w-full flex items-center justify-center gap-2 py-3 bg-[#FF6B6B]/10 text-[#FF6B6B] border border-[#FF6B6B]/30 rounded-xl text-sm font-black uppercase tracking-wider hover:bg-[#FF6B6B] hover:text-white transition-all"
                >
