...
// ✅ Updated: Ascento Playschool only — Abacus & Brain Development removed
...
function StudentFormFields({ form, setForm, programs, photoFile, setPhotoFile }: {
  ...
  // ✅ Fixed: correctly reads res.formatted, scoped to program+level+section
  useEffect(() => {
    if (!form.programId) return;
    const params = new URLSearchParams({ programId: form.programId });
    if (form.programLevelId) params.set("programLevelId", form.programLevelId);
    ...
    apiFetch(`/api/admin/students/next-roll-number?${params}`)
      .then((res) => {
        // res = { nextRollNumber: number, formatted: "01" }
...
// ── ID Card PDF HTML ────────────────────────────────────────────────────────────
// ✅ Fixed: landscape page, correct CR80 card size, both cards fully visible
async function buildIDCardHTML(student: any, logoUrl?: string): Promise<string> {
  let photoSrc = "";
  if (student.photoUrl) {
    const b64 = await urlToBase64(student.photoUrl);
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* Tall portrait card — matches modal preview exactly */
  .card {
    width: 260px;
    background: #fff;
    border-radius: 12px;
  ...
  .header-text .tag    { color:rgba(255,255,255,0.8); font-size:8px; line-height:1.3; }
  .header-text .adm    { color:rgba(255,255,255,0.7); font-size:7.5px; margin-top:2px; }
  ...
  .photo    { width:80px; height:90px; border-radius:50%; object-fit:cover; border:3px solid #e91e8c; }
  .photo-av { width:80px; height:90px; border-radius:50%; background:#f3e5f5; border:3px solid #e91e8c; display:flex; align-items:center; justify-content:center; font-size:28px; font-weight:900; color:#e91e8c; }

  .info { padding: 0 14px 6px; font-size:9.5px; }
  .row  { display:flex; gap:4px; margin-bottom:4px; align-items:flex-start; }
  .lbl  { font-weight:700; color:#333; width:52px; flex-shrink:0; }
  .val  { color:#555; font-weight:600; }
  .blood { color:#e91e8c !important; font-weight:900 !important; }
  ...
  .logo-av-back { width:48px; height:48px; border-radius:8px; background:linear-gradient(135deg,#e91e8c,#9c27b0); display:flex; align-items:center; justify-content:center; font-size:20px; font-weight:900; color:#fff; margin-bottom:6px; }
  ...
  .finder-title { font-weight:900; font-size:10px; color:#e91e8c; margin-bottom:4px; line-height:1.3; }
...
<div class="card-wrapper">
  ...
  <div class="card">
    ...
    <div class="wave-top">
      <svg viewBox="0 0 260 18"><path d="M0,18 Q65,0 130,10 Q195,20 260,4 L260,0 L0,0 Z" fill="#e91e8c" opacity="0.15"/></svg>
    </div>
    <div class="photo-wrap">${photoHtml}</div>
    <div class="info">
      <div class="row"><span class="lbl">Name</span><span class="val">: &nbsp;${student.fullName ?? "—"}</span></div>
    ...
    <div class="sign"><div class="sign-inner"><div class="sign-line"></div><div class="sign-label">Auth. Sign.</div></div></div>
    <div class="wave-bot">
      <svg viewBox="0 0 260 22"><path d="M0,22 L0,12 Q65,0 130,8 Q195,16 260,6 L260,22 Z" fill="#e91e8c"/><path d="M0,22 L0,16 Q65,4 130,12 Q195,20 260,10 L260,22 Z" fill="#9c27b0" opacity="0.6"/></svg>
    </div>
  </div>
</div>
...
<div class="card-wrapper">
  ...
  <div class="card">
    <div class="wave-top-back">
      <svg viewBox="0 0 260 28"><path d="M0,0 L260,0 L260,16 Q195,28 130,20 Q65,12 0,24 Z" fill="#9c27b0" opacity="0.6"/><path d="M0,0 L260,0 L260,10 Q195,22 130,14 Q65,6 0,18 Z" fill="#e91e8c"/></svg>
    </div>
    <div class="back-logo">
      ${logoImgBack}
      <div class="back-school">${SCHOOL_NAME}</div>
...
// ── Report PDF HTML ────────────────────────────────────────────────────────────
// ✅ Fixed: school name updated, admissionDate included
async function buildReportHTML(r: any): Promise<string> {
  const addr      = [r.address, r.city, r.state].filter(Boolean).join(", ") || "—";
  const enrolled  = r.enrolledAt    ? new Date(r.enrolledAt).toLocaleDateString("en-IN",    { year:"numeric", month:"long", day:"numeric" }) : "—";
  const admDate   = r.admissionDate ? new Date(r.admissionDate).toLocaleDateString("en-IN", { year:"numeric", month:"long", day:"numeric" }) : "—";
