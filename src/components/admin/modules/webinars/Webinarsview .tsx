...
// import {
//   Video, Plus, Search, Filter, Send, Edit2, Trash2, Bell,
//   Calendar, Clock, Users, Link2, Eye, X, ChevronDown,
//   Loader2, CheckCircle2, AlertCircle, Wifi, Play, Ban,
...
//   scheduled:  { label: "Scheduled",  color: "#2D8CFF", bg: "#EBF4FF", dot: "bg-blue-400" },
//   live:       { label: "Live",       color: "#22C55E", bg: "#F0FDF4", dot: "bg-green-400" },
//   completed:  { label: "Completed",  color: "#8B5CF6", bg: "#F5F3FF", dot: "bg-purple-400" },
//   cancelled:  { label: "Cancelled",  color: "#EF4444", bg: "#FEF2F2", dot: "bg-red-400" },
// };
...
//   programId: "", levelId: "", bannerUrl: "", sendEmail: false,
// });
...
//   // Track which webinar is currently sending what action
...
//       platform: w.platform, meetingLink: w.meetingLink,
//       meetingId: w.meetingId || "", passcode: w.passcode || "",
//       hostName: w.hostName || "", hostEmail: w.hostEmail || "",
//       scheduledAt: w.scheduledAt.slice(0, 16),
//       durationMins: w.durationMins,
//       programId: w.programId || "", levelId: w.levelId || "",
//       bannerUrl: w.bannerUrl || "", sendEmail: false,
//     });
...
//   // ── Send website notification only ────────────────────────────────────────
...
//       showToast(`🔔 Notifications sent to ${data.count} users!`);
//       fetchWebinars();
//     } catch (e: any) { showToast(e.message, "error"); }
...
//       const data = await res.json();
//       if (!res.ok) throw new Error(data.error || "Failed");
//       showToast(`📧 Emails sent to ${data.sent}/${data.total} recipients!`);
//       fetchWebinars();
//     } catch (e: any) { showToast(e.message, "error"); }
...
//       {/* Toast */}
//       {toast && (
//         <div className={`fixed top-5 right-5 z-[9999] flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-xl text-sm font-semibold border transition-all
//           ${toast.type === "success"
...
//           <p className="text-sm text-gray-500 mt-0.5">Schedule & manage online sessions for students and parents</p>
//         </div>
...
//     { label: "Scheduled", value: webinars.filter(w => w.status === "scheduled").length, color: "#2D8CFF", icon: Calendar },
//     { label: "Live",      value: webinars.filter(w => w.status === "live").length,      color: "#22C55E", icon: Wifi },
//     { label: "Completed", value: webinars.filter(w => w.status === "completed").length, color: "#8B5CF6", icon: CheckCircle2 },
//   ];
//   return (
...
//                       className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full cursor-pointer hover:opacity-80 transition-opacity"
//                       style={{ background: sm.bg, color: sm.color }}>
//                       <span className={`w-1.5 h-1.5 rounded-full ${sm.dot} ${w.status === "live" ? "animate-pulse" : ""}`} />
//                       {sm.label}
//                       <ChevronDown size={10} />
//                     </button>
//                     {showStatusMenu && (
//                       <div className="absolute top-full left-0 mt-1 z-20 bg-white border border-gray-100 rounded-xl shadow-lg py-1 min-w-[130px]">
//                         {(["scheduled", "live", "completed", "cancelled"] as WebinarStatus[]).map(s => (
...
//                   title="Send website notification to users">
...
//                   title="Send email to students & parents">
...
//                 <Link2 size={11} /> Join Link <ExternalLink size={10} />
//               </a>
//             </div>
...
//                     <BellRing size={11} /> In-app notifications sent
//                   </span>
//                 )}
//                 {w.emailSent && (
//                   <span className="flex items-center gap-1 text-xs text-green-600 font-semibold">
//                     <Mail size={11} /> Emails sent
//                   </span>
//                 )}
//               </div>
//             )}
...
//             <div className="grid grid-cols-2 gap-4">
//               <FormField label="Program (leave blank for all)">
...
//                 <p className="text-xs text-gray-500">Emails sent to all targeted students & parents</p>
//               </div>
//             </label>
//           )}
//         </div>
...
//       >
//         {/* Banner */}
//         <div className="rounded-t-2xl p-6 text-white relative overflow-hidden"
...
//                   <BellRing size={15} /> In-app notifications sent
//                   {w.notificationSentAt && (
//                     <span className="ml-auto text-xs text-purple-400 font-normal">
//                       {new Date(w.notificationSentAt).toLocaleString("en-IN")}
//                     </span>
//                   )}
//                 </div>
//               )}
...
//                   <Mail size={15} /> Email notifications sent
//                   {w.emailSentAt && (
//                     <span className="ml-auto text-xs text-green-500 font-normal">
//                       {new Date(w.emailSentAt).toLocaleString("en-IN")}
//                     </span>
//                   )}
//                 </div>
//               )}
...
//       <p className="text-sm text-gray-400 mb-5">Schedule your first online session for students and parents.</p>
