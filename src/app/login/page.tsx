...
//       // ── 3. Hydrate Supabase client session so middleware works ─────────────
//       await supabase.auth.setSession({
//         access_token:  result.access_token,
//         refresh_token: result.refresh_token,
//       });

//       // ── 4. Redirect — honour ?next= from middleware, else role-based ───────
//       const next = searchParams.get("next");

//       if (next) {
//         router.replace(next);                              // ← go back to intended page
//       } else {
//         router.replace(
...
//                 style={{ fontSize: 12, fontWeight: 800, color: "#FF6B6B", textDecoration: "none" }}
//               >
//                 Forgot password?
//               </Link>
//             </div>
...
//       <style>{`
//         @import url('https://fonts.googleapis.com/css2?family=Fredoka+One&family=Nunito:wght@400;600;700;800;900&display=swap');
//         .login-page { min-height:100vh; background:#FFFDF7; display:flex; align-items:center; justify-content:center; padding:40px 20px; position:relative; overflow:hidden; font-family:'Nunito',system-ui,sans-serif; }
//         .login-blob { position:absolute; border-radius:50%; filter:blur(80px); pointer-events:none; z-index:0; }
//         .login-card { position:relative; z-index:1; width:100%; max-width:460px; background:white; border-radius:32px; padding:48px 44px; box-shadow:0 24px 80px rgba(255,107,107,.12),0 4px 20px rgba(0,0,0,.06); border:2px solid rgba(255,107,107,.08); }
//         .login-icon { width:72px; height:72px; border-radius:22px; background:linear-gradient(135deg,#FF6B6B,#FFB347); display:flex; align-items:center; justify-content:center; font-size:36px; margin:0 auto 20px; box-shadow:0 12px 32px rgba(255,107,107,.35); }
...
//         .login-subtitle { font-size:14px; color:#999; text-align:center; font-weight:700; margin-bottom:32px; }
//         .login-error { background:#FFF0F0; border:2px solid #FFD6D6; color:#FF4444; border-radius:14px; padding:12px 16px; font-size:13px; font-weight:800; margin-bottom:20px; display:flex; align-items:center; gap:8px; }
//         .login-field { margin-bottom:18px; }
//         .login-label { display:block; font-size:10px; font-weight:900; letter-spacing:0.14em; text-transform:uppercase; color:#FF6B6B; margin-bottom:8px; margin-left:4px; }
//         .login-input-wrap { position:relative; }
...
//         .login-input:focus { border-color:#FF6B6B; box-shadow:0 0 0 4px rgba(255,107,107,.10); }
//         .login-input::placeholder { color:#CCC; font-weight:600; }
...
//         .login-submit { width:100%; padding:15px; border-radius:50px; border:none; background:linear-gradient(135deg,#FF6B6B,#FF8E53); color:white; font-family:'Fredoka One',cursive; font-size:18px; cursor:pointer; box-shadow:0 8px 24px rgba(255,107,107,.4); transition:all .3s cubic-bezier(.34,1.56,.64,1); display:flex; align-items:center; justify-content:center; gap:10px; margin-top:8px; }
//         .login-submit:hover:not(:disabled) { transform:scale(1.04) translateY(-2px); box-shadow:0 14px 32px rgba(255,107,107,.5); }
//         .login-submit:disabled { opacity:.65; cursor:not-allowed; }
...
//         .login-google:hover:not(:disabled) { border-color:#FFB347; box-shadow:0 4px 16px rgba(255,179,71,.2); transform:translateY(-2px); }
//         .login-google:disabled { opacity:.65; cursor:not-allowed; }
//         .login-footer { margin-top:28px; text-align:center; font-size:13px; color:#999; font-weight:700; }
//         .login-footer a { color:#FF6B6B; font-weight:900; text-decoration:none; }
//         .login-footer a:hover { text-decoration:underline; }
//         @keyframes spin { to { transform:rotate(360deg); } }
...
//             <div className="spinner" style={{ borderTopColor:"#FF6B6B", borderColor:"rgba(255,107,107,.2)" }} />
//           </div>
//         }>
//           <LoginForm />
//         </Suspense>
...
import Link from "next/link";
import { supabase } from "@/lib/helpers/supabaseClient";
import { syncCurrentUser } from "@/lib/auth/syncCurrentUser";
...
function LoginForm() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use role from DB response (Supabase session doesn't carry it)
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) throw error;
      // Redirect handled by /auth/callback — no syncCurrentUser needed here
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Google Sign-In failed.");
      setLoading(false);
    }
