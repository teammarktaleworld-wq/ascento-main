...
export async function GET(req: Request) {
  ...
  const typedRecords = attendanceRecords as { status: string; _count: number }[];

  const presentCount = typedRecords.find((r) => r.status === "present")?._count ?? 0;
  const totalCount = typedRecords.reduce((s, r) => s + r._count, 0);
  const attendancePct = totalCount ? Math.round((presentCount / totalCount) * 100) : 0;
  ...
  return NextResponse.json({
    ...
    upcomingExams,
    todaySchedule: todaySlots,
    enrollmentTrend: (enrollmentTrend as { month: string; count: bigint }[]).map((e) => ({
      month: e.month,
      count: Number(e.count),
    })),
  });
