...
// src/app/api/admin/teachers/[id]/status/route.ts
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    if (status === "Disabled") {
      // 1. Stamp forceLogoutAt — kicks any open browser tab on next /api/me poll
      ...
      // 2. Ban in Supabase — blocks future logins & invalidates all sessions
      const { error: banErr } = await supabaseAdmin.auth.admin.updateUserById(
        teacher.userId,
        { ban_duration: "876600h" }, // ~100 years = permanent
      );
      if (banErr) {
        console.error("[status] Supabase ban error:", banErr.message);
      ...
      // Activating: lift the Supabase ban first, then update DB
      const { error: unbanErr } = await supabaseAdmin.auth.admin.updateUserById(
        teacher.userId,
        { ban_duration: "none" },
      );
      if (unbanErr) {
        console.error("[status] Supabase unban error:", unbanErr.message);
      }
