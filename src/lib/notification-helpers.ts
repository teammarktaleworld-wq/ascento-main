...
// // Admin ALWAYS gets a copy. No exceptions.
// //
// // Webinar (no programId)  → ALL users (student, teacher, user, admin)
// // Webinar (with programId)→ enrolled students + all teachers + admin
...
//     // All teachers always get program webinars
...
//   // ── Always append admins, even when programId is set ─────────────────────
...
// // Same targeting logic but collects email strings instead of userIds.
// // Parent emails are included since parents don't have user accounts.
...
//     });
//     for (const t of teachers) {
//       results.push({ email: t.email, name: t.name ?? t.email });
//     }
//   } else {
//     // All user accounts
//     const users = await prisma.user.findMany({
...
//     // Parent emails (no user account — students table only)
//     const students = await prisma.student.findMany({
